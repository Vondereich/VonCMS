...
> **"Just upload, install, publish. Done."**
...
> [!WARNING]
> **🚨 CRITICAL UPDATE NOTICE (v1.11.12)**
> Users on version **v1.11.11 or older** MUST perform a **MANUAL UPDATE**.
>
...
> [!NOTE]
> **🏛️ Nara Foundation Complete — v1.20 Mandala is Next**
...
[![Latest Release](https://img.shields.io/badge/Release-v1.11.12-brightgreen?style=for-the-badge&logo=github)](https://github.com/Vondereich/VonCMS/releases)
[![Build Standard](https://img.shields.io/badge/Foundation-Nara_Complete-orange?style=for-the-badge)](https://github.com/Vondereich/VonCMS)
[![Next Version](https://img.shields.io/badge/Next-v1.20_Mandala-blueviolet?style=for-the-badge)](https://github.com/Vondereich/VonCMS)
...
[🌐 Official Website](https://skripglobal.com/) • [📥 Download v1.11.12](https://github.com/Vondereich/VonCMS/releases) • [📖 Full Docs](https://github.com/Vondereich/VonCMS/blob/main/Introduction.md)
...
VonCMS is built on three pillars:

1. **Security First**: Protection isn't a plugin; it's the foundation. A **6-layer defense** stack includes CSRF protection, session binding (anti-hijack), rate limiting, CORS origin allowlisting, server-side MIME validation, and self-healing integrity monitoring — all baked into a single `security.php` middleware.
...
3. **Flexibility Third**: A hybrid architecture (React + PHP) that runs anywhere. Whether it's a simple blog or a high-traffic news portal with **50,000+ posts per sitemap chunk** (auto-paginated), the system adapts to your scale without breaking.
...
**VonCMS Nara** eliminates the "Ghost Issues" common on cPanel and Shared Hosting. Instead of complex directory mapping, Nara uses **Absolute Path Hardening** — every file path resolves via `__DIR__`, never relative guesswork.
...
Production numbers from cPanel (skripglobal.com):
...
- **Deploy Package**: `< 1 MB` - Entire CMS fits in a single lightweight zip.

---

## 💎 Power Features

### 🔒 Security Stack (v1.11.12)

Every API endpoint is protected by a layered defense system built into the core:

- **Auth Gate**: Strict `$_SESSION['user']` validation on every write endpoint — no fallback, no bypass.
- **Bot-Proof API**: Social bot bypass is scoped to read-only page requests only. API POST endpoints are **never** bypassed, even with spoofed User-Agent.
- **CORS Hardening**: Origin allowlist with same-host verification. No wildcard + credentials combo.
- **Server-Side MIME**: Uploads validated via `finfo_file()` — client MIME spoofing is rejected.
- **Rate Limiting**: IP-based lockout (5 attempts / 15-min window) on all public endpoints.
- **CSRF Everywhere**: Every mutation requires a valid `X-CSRF-Token` header.

### 🌙 Neutral Dark Mode (v1.11.10)

Migrated from blue-tinted slate to a **Neutral Grey** palette, matching YouTube, X, and Facebook. Consistent across all 6 themes.
...
- **Chunked Sitemaps**: Auto-paginated at 50,000 URLs per chunk — handles 100K+ posts without breaking Google's limits.
- **Hydration Bridge**: Server-rendered initial state passed to React SPA for instant SEO-bot-friendly loads.
- **Atomic Transactions**: All write operations (post saves, slug changes, media deletes) use database transactions to prevent race conditions.
...
1. **Download**: Grab the `VonCMS_v1.11.12_Deploy.zip` from the releases page.
...
**Current Release**: v1.11.12 "Nara — Foundation Complete"  
**Next Version**: v1.20 "Mandala"  
**Main Showcase**: [skripglobal.com](https://skripglobal.com/)  
**API Endpoints**: 69 secured endpoints

_The Nara foundation is locked. Mandala is next._
