...
Current release: **v1.27.2 "OverDrive"**. You can install the Deploy ZIP on hosting, or fork the source repository to build your own themes, plugins, extensions, fixes, and release packages.
...
> [!NOTE]
> Before updating a production site to v1.27.2, back up its files and database, review the changelog, and test the Deploy ZIP on a local or staging installation when possible. After updating, verify the homepage, one post, one page, and the admin dashboard.
>
...
- `VonCMS_v1.27.2_Deploy.zip`
- `VonCMS_v1.27.2_Source.zip`
