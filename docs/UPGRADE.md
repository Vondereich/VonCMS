...
4. If your current site is on an older version, use the manual Deploy ZIP flow to upgrade to `v1.26.4`. OTA updates are available again from the `v1.24.10` baseline after the updater download and SHA256 verification flow was fixed. Sites on the affected `v1.25.11` through `v1.26.0` packages or the pre-fix `v1.26.1` package must complete this manual update once because Dashboard release discovery can be skipped.
...
6. If your site already passed the `v1.25.0` `.htaccess` repair step, update normally to `v1.26.4`.
