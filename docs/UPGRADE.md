...
4. If your current site is on an older version, use the manual Deploy ZIP flow to upgrade to `v1.25.6`. OTA updates are available again from the `v1.24.10` baseline after the updater download and SHA256 verification flow was fixed.
...
6. If your site already passed the `v1.25.0` `.htaccess` repair step, update normally to `v1.25.6`.
7. After the update, verify the homepage, one single post, and the admin dashboard.
8. After the site is already on `v1.25.0` with a repaired `.htaccess`, use the admin panel updater for future patches such as `v1.25.6`.

## What to verify after updating to v1.25.6
