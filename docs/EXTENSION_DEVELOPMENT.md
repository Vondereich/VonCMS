# VonCMS Extension Development Guide v1.26.7
...
VonCMS v1.26.7 separates normal appointed Admin access from primary-admin ownership. Extensions must respect that split.
...
The v1.26.7 public contract is:
...
Do not mirror one plugin's settings in multiple admin areas unless there is a current runtime owner for that split. The v1.26.7 baseline keeps per-extension config in Extensions, while site identity stays in General Settings.
