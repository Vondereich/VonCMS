...
> **Moving from shared hosting:** If you are already comfortable with cPanel and mainly need more capacity, consider a managed cPanel VPS and ask the hosting provider to complete the migration. Confirm before purchase that the service includes website files, database, DNS, SSL, the required PHP version and extensions, email and cron jobs when used, filesystem ownership, and the final web-server configuration. A generic file transfer alone is not a complete migration.
...
- `json`
- `gd`
- XML support through `DOM`, `XMLReader`, and `SimpleXML` (usually provided by the `php-xml` package)
...
- In the active PHP configuration, keep `upload_max_filesize` at least `64M` and `post_max_size` at least `70M`. The Nginx `70m` ceiling above allows the bounded 64 MB WordPress WXR import plus multipart overhead; VonCMS still applies its smaller endpoint-specific limits, including 10 MB for ordinary media and 50 MB for Database Manager imports. Do not set an unlimited request-body size.
- Confirm the active PHP-FPM configuration uses `cgi.fix_pathinfo=0`, limits executable script extensions to `.php`, and listens on a Unix socket or a loopback/private endpoint that is not publicly reachable. Keep aaPanel's generated handler and pool ownership unless you administer the complete PHP-FPM stack yourself.
...
- `fileinfo` is enabled
- `gd` is enabled
- Nginx `client_max_body_size`, PHP `upload_max_filesize`, and PHP `post_max_size` are aligned with the required upload size
