...
> VonCMS v1.26.5 feature baseline for the After Hours line.

## Introduction to VonCMS v1.26.5 "After Hours"
...
| Detail          | Value                                     |
| --------------- | ----------------------------------------- |
| Current release | `v1.26.5 "After Hours"`                   |
| Minimum PHP     | `8.2+`                                    |
...
VonCMS is built to stay light on disk and direct at runtime. The current `v1.26.5` release line keeps a small package surface, server-side pagination, indexed read paths for large content libraries, and a direct React-to-PHP-to-MySQL request path without a plugin-heavy middleware stack.
...
- **82 PHP API files** - current API surface under `public/api/`, with 99 public PHP files covered by the lint gate across the public runtime.
...
- **Light package surface** - current local `v1.26.5` release artifacts stay small for a full CMS package while keeping installer, docs, bundled themes, and self-hosted Inter font files intact.
