...
If the uploaded SQL contains the table-level `DROP TABLE IF EXISTS` or `CREATE TABLE` statements used by a VonCMS backup, VonCMS asks for explicit confirmation before running it. After confirmation, the backend creates a server-side pre-import safety backup in the protected `public/data/backups/` folder before it starts the destructive restore.

That safety backup is a fallback for admin mistakes, not a replacement for checking the target database first. VonCMS keeps at most five recent safety backups and removes files older than 30 days. Release packaging excludes the runtime backup directory.

The importer accepts only the unqualified table statements emitted by VonCMS Backup: `DROP TABLE IF EXISTS`, `CREATE TABLE`, `INSERT INTO`, and `SET FOREIGN_KEY_CHECKS` with a value of `0` or `1`. Database-qualified tables and broad server operations such as `DROP DATABASE`, `CREATE USER`, triggers, routines, events, or `SET GLOBAL` are rejected.
...
| `Only unqualified DROP TABLE IF EXISTS, CREATE TABLE...` | The SQL contains a statement or qualified table outside the exact VonCMS table-restore format.                                                                                             | Use a SQL file exported by VonCMS Backup. Use phpMyAdmin/MySQL CLI for full server-level dumps, database-qualified tables, custom DDL, or account/server operations.                                                    |
...
The SQL editor exposes only read operations supported by the backend: `SELECT`, `SHOW`, `DESCRIBE`, and `EXPLAIN`. Table creation/deletion and a client-side unsafe-mode switch are intentionally not offered.
