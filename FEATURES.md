...
> VonCMS v1.24.10 feature baseline for the HourGlass line.
...
Developer note: theme extension work is documented in [Theme Guide](THEME_GUIDE.md).
...
VonCMS is built to stay light on disk and direct at runtime. The current `v1.24.10` release line keeps a small package surface, server-side pagination and FULLTEXT baselines for large content libraries, and a direct React-to-PHP-to-MySQL request path without a plugin-heavy middleware stack.

Earlier Rentaka benchmark measurements were useful during tuning, but they are environment-specific and should be treated as directional engineering evidence, not a universal production guarantee. Final throughput depends on hosting tier, database shape, traffic mix, CDN or cache layers, and the exact endpoint under load.
...
- **Light package surface** — current local `v1.24.10` release artifacts stay in the sub-1MB class while keeping installer, docs, and bundled themes intact.
