...
> VonCMS v1.24.4 feature baseline for the HourGlass line.
...
VonCMS is built to stay light on disk and direct at runtime. The current `v1.24.4` release line keeps a small package surface, server-side pagination and FULLTEXT baselines for large content libraries, and a direct React-to-PHP-to-MySQL request path without a plugin-heavy middleware stack.
...
- **Light package surface** — current local `v1.24.4` release artifacts stay in the sub-1MB class while keeping installer, docs, and bundled themes intact.
