...
- **Loading Experience**: The production PHP entry, static HTML shell, React route fallback, and lazy public-theme handoff now render the same skeleton instead of exposing an empty mount or white theme-loading surface during cold JavaScript startup. Mobile retains three full-width cards, tablets add a fourth tablet-only card for a balanced 2x2 grid, and desktop keeps three compact centered cards; one shared stylesheet owns all breakpoint sizing, palette, shimmer, accessible busy, and reduced-motion behavior without duplicate runtime style injection.
