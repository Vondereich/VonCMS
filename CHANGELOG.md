...
- **Loading Experience**: The production PHP entry, static HTML shell, React route fallback, and lazy public-theme handoff now render the same skeleton instead of exposing an empty mount or white theme-loading surface during cold JavaScript startup. Mobile and desktop retain three cards, while tablets add a fourth tablet-only card to complete a balanced 2x2 grid; one shared stylesheet owns all breakpoint sizing, palette, shimmer, accessible busy, and reduced-motion behavior without duplicate runtime style injection.
