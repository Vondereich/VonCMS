...
- **Security Hardening**:
  ...
  - **Safe Image Sink Boundary**: Public/admin avatar and preview image surfaces now render through a centralized `SafeImage` component with fail-closed URL normalization, load-error fallback, and a single audited `React.createElement` image sink for the remaining CodeQL DOM reinterpretation alerts.
