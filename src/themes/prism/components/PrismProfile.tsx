...
const GlitchText: React.FC<{ text: string; className?: string }> = ({ text, className }) => (
  <span className={`relative inline-block group ${className} `}>
    ...
    <span className="absolute top-0 left-0 -z-10 w-full h-full text-(--color-secondary) opacity-0 group-hover:opacity-70 group-hover:translate-x-[2px] transition-all duration-100 select-none">
      {text}
    </span>
    <span className="absolute top-0 left-0 -z-10 w-full h-full text-(--color-primary) opacity-0 group-hover:opacity-70 group-hover:translate-x-[-2px] transition-all duration-100 select-none">
      {text}
    </span>
  </span>
);
    ...
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      {isEditing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fade-in">
          <div className="bg-[#0a0a1f] border border-(--color-primary) rounded-2xl shadow-[0_0_50px_rgba(139,92,246,0.3)] w-full max-w-md overflow-hidden relative">
            {/* Decorative Lines */}
            <div className="absolute top-0 left-0 w-full h-[2px] bg-linear-to-r from-transparent via-(--color-primary) to-transparent"></div>

            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <h3 className="text-xl font-bold text-white flex items-center gap-2">
                <Terminal size={18} className="text-(--color-primary)" />
                EDIT_PROFILE_DATA
              </h3>
            </div>
            <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto custom-scrollbar">
              <div>
                <span className="block text-xs font-mono text-(--color-secondary) mb-2 uppercase tracking-wider">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-sm bg-black/50 border border-white/10 text-white font-mono focus:border-(--color-primary) focus:outline-hidden focus:shadow-[0_0_15px_rgba(139,92,246,0.2)] transition-all"
                />
              </div>
              <div>
                <span className="block text-xs font-mono text-(--color-secondary) mb-2 uppercase tracking-wider">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-sm bg-black/50 border border-white/10 text-white font-mono focus:border-(--color-primary) focus:outline-hidden focus:shadow-[0_0_15px_rgba(139,92,246,0.2)] transition-all resize-none"
                />
              </div>
              ...
              <div className="pt-4 border-t border-white/10">
                <button
                  ...
                  className="text-xs font-mono font-bold text-(--color-primary) mb-4 flex items-center gap-2 hover:text-white transition-colors tracking-widest"
                >
                  [{showPasswordFields ? '-' : '+'}]{' '}
                ...
                {showPasswordFields && (
                  <div className="space-y-4 animate-fade-in p-4 border border-(--color-primary)/30 bg-(--color-primary)/5 rounded-lg">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 rounded-sm bg-black border border-white/10 text-white font-mono focus:border-red-500 focus:outline-hidden"
                      />
                    </div>
                    <div>
                      <span className="block text-xs font-mono text-(--color-secondary) mb-1 uppercase tracking-wider">
                        New Password
                      </span>
                      <input
                        id="prismprofile-241"
                        ...
                        className="w-full px-3 py-2 rounded-sm bg-black border border-white/10 text-white font-mono focus:border-(--color-primary) focus:outline-hidden"
                      />
                    </div>
                    <div>
                      <span className="block text-xs font-mono text-(--color-secondary) mb-1 uppercase tracking-wider">
            ...
            <div className="p-6 border-t border-white/10 bg-white/5 flex justify-end gap-3">
              ...
              <button
                ...
                className="px-6 py-2 bg-(--color-primary)/20 border border-(--color-primary) text-(--color-primary) hover:bg-(--color-primary) hover:text-black transition-all font-mono text-sm font-bold tracking-wider"
              >
                SAVE_CHANGES
              </button>
            </div>
        ...
        <div className="flex gap-3">
          {isOwnProfile && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/20 text-white rounded-sm hover:bg-white/10 transition-all font-mono text-xs font-bold tracking-wider"
            >
              <Terminal size={14} />
              EDIT_DATA
            </button>
          ...
          {canAccessAdmin && onNavigateAdmin && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-(--color-primary)/10 border border-(--color-primary) text-(--color-primary) rounded-sm hover:bg-(--color-primary) hover:text-black transition-all font-mono text-xs font-bold tracking-wider"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-[#0a0a1f]/50 backdrop-blur-xs border border-white/10 rounded-2xl p-6 relative overflow-hidden group">
            <div className="absolute inset-0 bg-linear-to-b from-(--color-primary)/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>

            <div className="relative z-10 flex flex-col items-center text-center">
              <div className="w-32 h-32 rounded-full p-1 bg-linear-to-br from-(--color-primary) to-(--color-secondary) mb-4 relative">
                <div className="absolute inset-0 rounded-full bg-(--color-primary) blur-md opacity-40 animate-pulse"></div>
                <SafeImage
                  src={avatarSrc}
          ...
          {/* Contact / Bio */}
          <div className="bg-[#0a0a1f]/50 backdrop-blur-xs border border-white/10 rounded-2xl p-6">
            <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
              <Terminal size={14} className="text-(--color-secondary)" />
              USER_DATA
            </h3>
            <div className="space-y-3 text-sm text-slate-400 font-mono">
              ...
              {displayUser.bio && (
                <div className="pt-3 mt-3 border-t border-white/10">
                  <span className="block text-(--color-secondary) mb-1">BIO_MESSAGE:</span>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="flex gap-8 border-b border-white/10 mb-6">
            ...
            >
              <Cpu
                className={activeTab === 'articles' ? 'text-(--color-primary)' : 'opacity-50'}
                size={20}
              />
              PUBLISHED_CONTENT
              {activeTab === 'articles' && (
                <span className="absolute bottom-0 left-0 w-full h-[2px] bg-(--color-primary) shadow-[0_0_10px_rgba(139,92,246,0.5)]"></span>
              )}
            </button>
            <button
            ...
            >
              <Terminal
                className={activeTab === 'comments' ? 'text-(--color-primary)' : 'opacity-50'}
                size={20}
              />
              USER_DISCUSSION
              {activeTab === 'comments' && (
              ...
              <>
                {commentItems.length > 0 ? (
                  commentItems.map((comment: any) => (
                    <div
                      key={comment.id}
                      className="bg-[#0a0a1f]/30 border border-white/5 rounded-xl p-5 hover:border-(--color-primary)/30 transition-all"
                    >
