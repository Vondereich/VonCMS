...
export const EditorPreviewModal = ({ isOpen, html, onClose }: EditorPreviewModalProps) => (
  ...
  >
    <div className="flex max-h-[calc(100dvh-1.5rem)] w-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-admin-border dark:bg-admin-panel sm:max-h-[90dvh]">
      <div className="p-4 border-b border-slate-100 dark:border-admin-border bg-slate-50 dark:bg-admin-canvas/50 flex justify-between items-center">
        ...
        <button
          type="button"
          onClick={onClose}
          className="flex h-11 w-11 items-center justify-center rounded-full text-slate-400 transition-colors hover:bg-slate-100 hover:text-red-500 dark:hover:bg-admin-hover"
